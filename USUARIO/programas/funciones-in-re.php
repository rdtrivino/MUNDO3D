...
if (!$result) {
    ...
    header("Location: ../serviciodeimpresion.php");
    exit(); // Terminamos la ejecución del script después de redirigir
}
...
// Procesamos los datos del formulario si es un POST
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] == 'vaciar_carrito') {
            ...
            if ($result) {
                ...
                echo "Error al obtener la cantidad actual: " . mysqli_error($link);
            }
            exit();
    ...
    if (isset($_POST['producto'])) {
        ...
        if ($producto) {
            ...
            if ($result_check) {
                ...
                echo "Error al verificar el producto: " . mysqli_error($link);
            }
            exit();
        } else {
    ...
    if (isset($_FILES['archivoImpresion'])) {
        ...
        if ($resultado_estado) {
            ...
            $sql = "INSERT INTO pedidos (Pe_Cliente, pe_nombre_pedido, pe_tipo_impresion, nombre_imagen, pe_color, Pe_Observacion, Pe_Cantidad, Pe_Estado, Pe_FechaPedido, Pe_FechaEntrega, Pe_Producto) VALUES ('$cliente', '$nombrePedido', '$tipoImpresion', '$imagenBinaria', '$color', '$observaciones', '$cantidad', 1, '$fechaPedido', '$fechaEntrega', 4)";
            ...
            echo "Error al verificar el estado del pedido: " . mysqli_error($link);
        }
    }
